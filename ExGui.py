...
class Experiment(object):
    ...
    SCALE_VARIABLES = {}
    ALL_ENTRIES = {}
    QUESTIONS_TEXT_LABELS = {} # frame_n : {...}} ## frame name is a string.
    BUTTONS = {}
    ...
    def create_question(
        ...
        entry_local.insert(0, default_value)
    ...
    def create_frame(
        ...
        if parent != None:
            parent = type(self).ALL_FRAMES[parent] # parent is a string of frame name - in case parent is another frame
        else:
            parent = type(self).EXPERIMENT_GUI
        
        if full_screen == True: 
            frame = tk.Frame(parent, width = self.x, height = self.y) # creation of current frame
        else:           
        ...
        type(self).LABELS_BY_FRAMES[frame_name] = {} # example: 'label_1' : <tk.Label object at 0xd23ff5d>
    ...
    def create_label(
        ...
        if blank_label == False and image_label == False:
            ...
            if frame_name in type(self).ALL_FRAMES.keys():
                ...
                type(self).LABELS_BY_FRAMES[frame_name] = {label_name : label} # check if nesseary
    ...
    def display_frame(self, frame_name, labels_order, pack_style=None, use_place=None): 
    # use_place is a dictionary that contains label name as key and a tuple of coordinates x and y)
        for label in labels_order: # iteration over nest labels
            if use_place != None: # if use_place is specified at all
                if label in use_place.keys(): # if current label is specified in use_place
                    if label in type(self).LABELS_BY_FRAMES[frame_name].keys(): # if label is actually a label
                        type(self).LABELS_BY_FRAMES[frame_name][label].place(x = use_place[label][0], y = use_place[label][1], anchor='center')
                    else: # in case this "label" in labels_order is actually a frame nested within the frame to be displayed
                        type(self).ALL_FRAMES[label].place(x = use_place[label][0], y = use_place[label][1], anchor='center')
                else: # if label is not in use_place 
                    ...
                    else: # this is aimed to deal with frames within frames
                        type(self).ALL_FRAMES[label].pack()
            else:
                if label in type(self).LABELS_BY_FRAMES[frame_name].keys(): # label is label
                    type(self).LABELS_BY_FRAMES[frame_name][label].pack(side=pack_style, expand=1)
                else: # this is aimed to deal with frames within frames
                    type(self).ALL_FRAMES[label].pack() 
