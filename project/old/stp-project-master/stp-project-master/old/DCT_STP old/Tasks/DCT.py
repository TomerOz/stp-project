...
class DctTask(object):
	...
	def start_task(self):
		...
		print self.td.catch_trials
		print self.td.correct_catch_trials
...
class TaskData(object):
	...
	def __init__(self, menu, phase=None):
	...
	def _split_train_and_post_trials(self):
		...
		self.training_neutralss = self.neutral_sentences[0:half_nutrals]
		...
		self.pre_intervention_sentences = self.training_neutralss + self.training_negatives
	...
	def _redefine_sentences_according_to_phase(self, phase=None):
		...
		if phase == 'training':
			...
			self.neutral_sentences = self.training_neutralss
