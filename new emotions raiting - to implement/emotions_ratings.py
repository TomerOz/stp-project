...
def background_emotions(win):
    path_background = "./input/graphics/instructions/emotions_ratings/1.jpg"
    background_image = visual.ImageStim(win=win,image=path_background, units='norm')
    # background_image.autoDraw = True
    return background_image

def endButton(win):
    path_button = "./input/graphics/instructions/emotions_ratings/finish_green.png"
...
def rating(win,dic_rect,background_image, block, additional_info):
...
def emotion_rating(win,block,additional_info):
    ...
    rating(win, dic_rect,background_image,block, additional_info)
    # win.close()
