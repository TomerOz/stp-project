...
class EmotionalReactivityTask(object):
	def __init__(self, er_gui, e_data_task, experimental_phase):
		...
		for emotion in self.e_data_task.keys():
	...
	def run_task(self):
		...
		places = []
		for place in self.er_gui.scale_places.keys():
			places.append(place)
		disp_list = [EMOTION_LABEL, EXTREME_RIGHT_E_LABLE, EXTREME_LEFT_E_LABLE, BUTTON_LABEL] + places
...
class EmotionalReactivityGui(object):
	...
	def create_canvases(self):
		...
		for i in range(self.scale_range):
			...
			canvas = self.exp.tk_refference.Canvas(frame, bg="grey", width = self.er_level_box_width, height=self.boxes_heights, bd=0,  highlightthickness=0)
